...
function _turnMicDeviceOn(userID, on) {
  return new Promise((resolve, reject) => {
    if (_isLocalUser(userID)) {
      ...
      _notifyUserCountOrPropertyChanged(ZegoChangedCountOrProperty.microphoneStateUpdate); // sync device status via stream extra info
...
function _turnCameraDeviceOn(userID, on) {
  return new Promise((resolve, reject) => {
    if (_isLocalUser(userID)) {
      ...
      _notifyUserCountOrPropertyChanged(ZegoChangedCountOrProperty.cameraStateUpdate); // sync device status via stream extra info
...
function _registerEngineCallback() {
  ...
  ZegoExpressEngine.instance().on('remoteCameraStateUpdate', (streamID, state) => {
    zloginfo('[remoteCameraStateUpdate callback]', streamID, state); // 0 for device is on
  ...
  ZegoExpressEngine.instance().on('remoteMicStateUpdate', (streamID, state) => {
    zloginfo('[remoteMicStateUpdate callback]', streamID, state); // 0 for device is on
...
function _tryStartPublishStream() {
  if (_localCoreUser.isMicDeviceOn || _localCoreUser.isCameraDeviceOn) {
    ...
    ZegoExpressEngine.instance().startPublishingStream(_localCoreUser.streamID).then(() => {
      zloginfo('Notify local user audioVideoAvailable start', _localCoreUser.streamID + '', JSON.parse(JSON.stringify(_streamCoreUserMap))); // if (_localCoreUser.streamID in _streamCoreUserMap) {
